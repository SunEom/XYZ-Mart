...
const ItemPresenter = ({ img, brand, serial, cost, isNew, id }) => {
  return (
    <div style={{ width: 285 }} className="font-mont">
      <Link to={`/product/${id}`}>
        ...
        <div className="h-20 flex flex-col justify-center px-2">
          <div className="font-bold pt-2 pb-1">{brand}</div>
          <div className="uppercase text-sm">{serial}</div>
        </div>
        <div className="h-auto flex flex-col justify-center px-2">
          <div className="font-bold py-2 text-lg">{new Intl.NumberFormat().format(cost)}원</div>
